...
const data = reactive({
  ...
  // 表单验证规则
  rules: {
    number: [{ required: true, message: '消息数不能为空' }],
  },
});
...
const handleExport = () => {
  ...
  proxy
      .download('/ai/item/export', { ...queryParams.value }, 'AI会话数据')
      .finally(() => {
...
<template>
  <div class="app-container">
    <Breadcrumb :items="['menu.ai.qa', 'menu.ai.item.list']" />
    <a-card class="general-card" :title="$t('menu.ai.item.list')">
      <!-- 头部区域 -->
      <div class="header">
        <!-- 搜索栏 -->
      ...
      >
        <template #columns>
          ...
          <a-table-column title="会话名称" data-index="name" />
      ...
      >
        <a-form ref="formRef" :model="form" :rules="rules" size="large">
          ...
          <a-form-item label="最后一条消息" field="lastMessage">
            <a-input v-model="form.lastMessage" placeholder="请输入最后一条消息" />
          </a-form-item>
