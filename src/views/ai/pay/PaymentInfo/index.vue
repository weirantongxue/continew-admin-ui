    ...
    <a-card class="general-card" :title="$t('menu.ai.pay.info.list')">
      ...
      <div class="header">
        ...
        <div v-if="showQuery" class="header-query">
          <a-form ref="queryFormRef" :model="queryParams" layout="inline">
            ...
            <a-form-item field="transactionId" hide-label>
              <a-input
                  v-model="queryParams.transactionId"
                  placeholder="输入支付系统交易编号搜索"
                  allow-clear
                  style="width: 150px"
                  @press-enter="handleQuery"
              />
            </a-form-item>
            <a-form-item field="paymentType" hide-label>
              <a-input
                  v-model="queryParams.paymentType"
                  placeholder="输入支付类型搜索"
                  allow-clear
                  style="width: 150px"
                  @press-enter="handleQuery"
              />
      ...
      >
        <a-descriptions :column="2" bordered size="large">
          ...
          <a-descriptions-item label="支付系统交易编号">
            <a-skeleton v-if="detailLoading" :animation="true">
              <a-skeleton-line :rows="1" />
            </a-skeleton>
            <span v-else>{{ dataDetail.transactionId }}</span>
          </a-descriptions-item>
          <a-descriptions-item label="支付类型">
            <a-skeleton v-if="detailLoading" :animation="true">
              <a-skeleton-line :rows="1" />
            </a-skeleton>
            <span v-else>{{ dataDetail.paymentType }}</span>
          </a-descriptions-item>
          <a-descriptions-item label="交易类型">
            <a-skeleton v-if="detailLoading" :animation="true">
              <a-skeleton-line :rows="1" />
            </a-skeleton>
            <span v-else>{{ dataDetail.tradeType }}</span>
          </a-descriptions-item>
          ...
          <a-descriptions-item label="支付金额(分)">
            ...
            <span v-else>{{ dataDetail.payerTotal }}</span>
          </a-descriptions-item>
          <a-descriptions-item label="通知参数">
            <a-skeleton v-if="detailLoading" :animation="true">
              <a-skeleton-line :rows="1" />
            </a-skeleton>
